function [K,rho,feas]=LMI_CT_DeDicont_Disk_ControlEffort(A,B,C,N,ContStruc)
...
LMIconstr=[(rho^2-alpha^2)*Y-A*Y*A'-Btot*L*A'-A*L'*Btot'-alpha*(Y*A'+L'*Btot'+Btot*L+A*Y) Btot*L;
    L'*Btot' Y]>=1e-2*eye(2*ntot);

LMIconstr2 = [gammaY*eye(ntot) eye(ntot); eye(ntot) Y]>=1e-2*eye(2*ntot);

LMIconstr3 = [gammaL*eye(ntot) L'; L eye(mtot)]>=1e-2*eye(ntot+mtot);

LMIconstr4 = [Y>=1e-2*eye(ntot)];

options=sdpsettings('solver','sedumi');
J=optimize([LMIconstr,LMIconstr2,LMIconstr3, LMIconstr4],[p1*gammaL+p2*gammaY],options);
feas=J.problem;
L=double(L);
Y=double(Y);
