function [K,rho,feas]=LMI_CT_DeDicont_Dregion(A,B,C,N,ContStruc)
...
LMIconstr=[sin(theta)*(A*Y+Y*A'+Btot*L+L'*Btot') cos(theta)*(A*Y-Y*A'+Btot*L-L'*Btot');
    cos(theta)*(Y*A'-A*Y+L'*Btot'-Btot*L) sin(theta)*(A*Y+Y*A'+Btot*L+L'*Btot')]<=1e-2*eye(2*ntot);

LMIconstr2 = [Y*A'+A*Y+L'*Btot'+Btot*L+2*alpha*Y]<=1e-2*eye(ntot);

Yconstr = Y>=1e-2*eye(ntot);

options=sdpsettings('solver','sedumi');
J=optimize([Yconstr, LMIconstr, LMIconstr2],[],options);
feas=J.problem;
L=double(L);
Y=double(Y);
