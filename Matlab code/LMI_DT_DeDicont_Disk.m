function [K,rho,feas]=LMI_DT_DeDicont_Disk(F,G,H,N,ContStruc)
...
LMIconstr=[[(rho^2-alpha^2)*P-F*P*F'-Gtot*L*F'-F*L'*Gtot'-alpha*(P*F'+L'*Gtot'+Gtot*L+F*P) Gtot*L;
    L'*Gtot' P]>=1e-2*eye(ntot*2)]+[P>=1e-2*eye(ntot)];

Pconstr = P>=1e-2*eye(ntot);
